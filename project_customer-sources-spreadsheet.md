...
Technical notes: no Python on this PC — append via Excel COM in PowerShell (`New-Object -ComObject Excel.Application`; find last used row, write cells, save). Value2 assignment throws InvalidCastException in PS 5.1 COM binding even WITH explicit `[string]`/`[double]` casts (confirmed 8/5/26 on the $12.46 double) — assign `.Formula = "12.46"` (string) instead; Excel parses it to a number and NumberFormat still applies. Body font Arial 10; column G format `$#,##0.00`. Related: [[koozie-pricing-calculator-page]], [[draft-order-from-quote-request]].
