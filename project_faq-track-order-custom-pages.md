...
- To add/edit questions: pageUpdate the body (each Q = details block + matching JSON-LD entry). Best route (8/4/26): `shopify store execute -s qualityperfection.myshopify.com --query-file X --variable-file Y --allow-mutations` — CLI store auth stored for sales@ with read/write_content + read/write_legal_policies scopes (the MCP connector lacks write_legal_policies, and note CLI JSON output has NO `data` wrapper). Same route works for shop policy edits (shopPolicyUpdate).
