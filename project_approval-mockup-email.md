...
4. Create a Gmail **draft** (never auto-send) to the customer's email. **GOTCHA (order #8778, 7/20/26): Gmail STRIPS remote `<img>` tags from API-created drafts — the first approval email went out with an empty cell where the mockup was.** Template now includes a visible "view your mockup" fallback link under the image; ALWAYS verify the image survived (open the draft in the browser, or get_thread after send) before/after sending. Host mockups as theme assets (assets/qp-approval-<order>.png via CLI push — Files API not needed). Gmail MCP has NO send tool — merchant clicks Send, or drive Chrome: Gmail account is u/2 (contact@) in their profile (u/0=yanivbiz1, u/1=qpllc1). Subject: `Your Mockup Is Ready for Approval — Order #XXXX | QualityPerfection`. Green Approve button = mailto:sales@quality-perfection.com with pre-filled "APPROVED — Order #XXXX" reply; secondary "Request changes" mailto link.
