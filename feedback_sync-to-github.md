...
- **Second, separate repo — don't confuse them:** `QPOWNER/claude-history-backup` (private, first backup 7/26/26) holds the full **session transcripts**, staged from `C:\Users\qpllc\claude-history-backup` (robocopy `.claude\projects` → `projects\`, delete nested `.git`, commit, push). That one is a one-way archive of conversations; **this** repo (`claude-memory`) is the two-way memory sync. "Update the info from GitHub" almost always means pull `claude-memory`.
