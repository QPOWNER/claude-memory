...
**HOSTED & LIVE (7/19/26):** backend deployed on Railway — project `koozie-customizer` id 7f6e1a6b-5d89-4e95-b8ab-23018db2f766, service koozie-customizer, domain https://koozie-customizer-production.up.railway.app, volume at /data (DATABASE_URL=file:/data/prod.sqlite; prisma url now env()). Railway CLI authed as contact@ (browserless pairing; MSYS_NO_PATHCONV=1 needed for -m /data on Git Bash). Env vars set from `shopify app env pull` .env. shopify.app.toml → production URLs (v7 deployed), automatically_update_urls_on_dev=false so `shopify app dev` won't clobber prod URLs. Embedded dashboard + Plan & billing page verified rendering in test-store admin; user approved a Pro test subscription (billing round trip works). Deploy code changes with: `railway up --detach` from app dir. API secret ROTATED 7/19 (old leaked-in-transcript secret revoked; new secret synced to Railway via env pull → variables --set, never displayed; Shopify rotation is two-phase: generate new + revoke old).

**Listing package (7/19/26 evening):** public name = "QP Product Customizer" (user chose; avoid "Koozie" in public name — BIC trademark; toml name updated, v8 deployed). Privacy policy live at /privacy (route app/routes/privacy.jsx; no-auth public page). Icon: Documents\koozie-customizer-app\listing-assets\app-icon-1200.png (SVG→sharp, teal + can cooler + 4 tool tiles + QP). Full listing copy drafted: listing-assets\listing-copy.md (tagline, description, features, pricing, search terms, reviewer instructions). Support email contact@ for now (changeable anytime). STILL TODO before submission: screenshots (1600×900, needs foregrounded browser w/ user), Starter 10-product enforcement (or soften plan copy), final compliance pass, create listing in dev dashboard + submit.
